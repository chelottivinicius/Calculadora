...
1. Criar markup HTML semântico e acessível (display, botões, atributos ARIA).
2. Implementar layout CSS inspirado no iPhone: grid de botões, cantos arredondados, sombras, fontes, responsividade.
3. Implementar lógica JS: entrada de números, operações, cálculo, limpar, ponto decimal, porcentagem, ± e lidar com erros (divisão por zero).
4. Testes responsivos: testar em desktop, tablet e mobile; ajustar breakpoints e tamanhos de botão.
5. Acessibilidade: suporte a teclado, foco visível, contraste e labels ARIA.
6. Finalizar artefatos: otimizar CSS/JS, empacotar arquivos estáticos e gerar instruções simples de deploy (GitHub Pages / copiar para site).
